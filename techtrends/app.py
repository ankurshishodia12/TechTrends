...
# Define metrics for web applications
...
def post(post_id):
    ...
    if post is None:
        app.logger.info(str(datetime.datetime.now()) +
                        " Page doesn't exist, 404 Not Found")
        return render_template('404.html'), 404
    else:
        app.logger.info(str(datetime.datetime.now()) +
                        ' , Article "' + post['title'] + '" is retrieved!')
        return render_template('post.html', post=post)

# Define the About Us page
@app.route('/about')
def about():
    app.logger.debug(str(datetime.datetime.now()) + " About Us page is retrieved!")
